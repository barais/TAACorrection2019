...
/**
 * REST controller for managing {@link fr.istic.taa.domain.Task}.
 */
@RestController
@RequestMapping("/api")
public class TaskResource {
    ...
    public TaskResource(TaskService taskService, TaskRepository taskRepository, TODOListRepository todoListRepository) {
    ...
    @PutMapping("/todos/{todolistid}/{dateDebut}/{dateFin}")
    public ResponseEntity<List<Task>> getTaskByDate(
        @PathVariable(value = "todolistid", required = true) final Long id,
        @PathVariable(value = "dateDebut", required = true) final String dateDebut,
        @PathVariable(value = "dateFin", required = true) final String dateFin
    ) throws URISyntaxException {
        List<Task> tasks =
            this.todoListRepository.findTaskForTodoListBetween2Date(id, LocalDate.parse(dateDebut), LocalDate.parse(dateFin));

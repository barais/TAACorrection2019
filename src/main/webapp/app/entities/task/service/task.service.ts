...
export class TaskService {
  ...
  protected convertDateFromClient(task: ITask): ITask {
    return Object.assign({}, task, {
      dateButoire: task.dateButoire?.isValid() ? task.dateButoire.format(DATE_FORMAT) : undefined,
    });
  }

  protected convertDateFromServer(res: EntityResponseType): EntityResponseType {
    if (res.body) {
      res.body.dateButoire = res.body.dateButoire ? dayjs(res.body.dateButoire) : undefined;
    }
    return res;
  }
  ...
  protected convertDateArrayFromServer(res: EntityArrayResponseType): EntityArrayResponseType {
    if (res.body) {
      res.body.forEach((task: ITask) => {
        task.dateButoire = task.dateButoire ? dayjs(task.dateButoire) : undefined;
      });
    }
    return res;
  }
